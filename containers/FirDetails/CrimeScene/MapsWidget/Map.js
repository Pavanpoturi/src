...
import { isEqual, min, max, isEmpty } from "lodash";
...
class Map extends Component {
  ...
  fitMarkersIntoBounds = () => {
    if (isEmpty(this.positions)) return false;
    let validLatList = this.positions.map((position) => position[0]);
    let validLongList = this.positions.map((position) => position[1]);
    let southWestBound = new L.LatLng(max(validLatList), max(validLongList));
    let northEastBound = new L.LatLng(min(validLatList), min(validLongList));
...
class MapMarker extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (isEqual(prevProps.position, this.props.position)) return false; //Render new Marker only if positions have changed
    this.renderMarker();
  }
...
Map.defaultProps = {
  ...
  location: false,
  zoom: 12,
  height: "300px", //Accepts Percentages also
  width: "290px",
};
