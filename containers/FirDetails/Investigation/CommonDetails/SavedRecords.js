...
export default function SavedRecords({
  ...
  const columns = [
    ...
    {
      title: "Place of Inquest",
      dataIndex: "selectedRecord",
      rowKey: "selectedRecord",
      render: (selectedRecord) => (
        <span className="tableRowText wordWrap">
          {selectedRecord?.placeOfInquest}
        </span>
      ),
    },
    {
      ...
      render: (_i, item) => {
        return (
          <div
            ...
            onClick={() =>
              getFileById(
                item?.selectedRecord?.inquestFormURL?.fileId,
                item?.selectedRecord?.inquestFormURL?.name,
                item?.selectedRecord?.inquestFormURL?.url
              )
            }
          >
            {item?.selectedRecord?.inquestFormURL?.name}
