...
export default function JuvenileApprehension({
  ...
  const JuvenileApprehensionData = useSelector(
  ...
  const [selectedCCLDataList, setSelectedCCLDataList] = useState([]);
  ...
  const getAccusedDropdownData = () =>
    getAccuseds(cclListDropDown(filterListArrest));
  ...
  const isSuccess =
    ...
    JuvenileApprehensionData?.actionType ===
      "ADD_JUVENILE_APPREHENSION_SUCCESS" ||
    JuvenileApprehensionData?.actionType ===
      "UPDATE_JUVENILE_APPREHENSION_SUCCESS";

  const isError =
    actionType === "ARREST_UPDATE_ERROR" ||
    actionType === "ARREST_CREATE_ERROR" ||
    JuvenileApprehensionData?.actionType ===
      "ADD_JUVENILE_APPREHENSION_ERROR" ||
    JuvenileApprehensionData?.actionType ===
      "UPDATE_JUVENILE_APPREHENSION_ERROR";
  ...
  useEffect(() => {
    if (isSuccess || isError) {
      ...
      } else if (errorMessage || JuvenileApprehensionData?.errorMessage) {
        if (!!errorMessage) {
          openNotificationWithIcon("error", errorMessage);
        } else {
          openNotificationWithIcon(
            "error",
            JuvenileApprehensionData?.errorMessage
          );
        }
      }
      if (
  ...
  }, [actionType, JuvenileApprehensionData]);
  ...
  useEffect(() => {
    var cclDataList = JSON.parse(
      JSON.stringify(JuvenileApprehensionData?.juvenileApprehensionList)
    );
    for (let i = 0; i < cclDataList?.length; i++) {
      if (
        !!cclDataList[i]?.typeOfApprehension &&
        cclDataList[i]?.typeOfApprehension?.toLowerCase() ===
          "Apprehension By Police"?.toLowerCase()
      ) {
        var obj = {
          crimeId: cclDataList[i]?.crimeId,
          accusedId: cclDataList[i].accusedId,
          action: cclDataList[i].action,
          arrestType: cclDataList[i].typeOfApprehension,
          arrestByPolice: {
            placeOfTakenIntoCustody: cclDataList[i]?.placeOfApprehension,
            dateTimeOfCustody: cclDataList[i].bringingDate,
            apprehendedBy: cclDataList[i].placeOfApprehension,
            accusedCode: cclDataList[i]?.cclCode,
            placeOfArrest: cclDataList[i].placeOfArrest,
            courtName: cclDataList[i].courtName,
            dateAndTimeOfArrest: cclDataList[i].apprehensionDate,
            intimationOfArrest: {
              relationToAccused:
                cclDataList[i]?.intimationGivenTo?.relationToCCL,
              selectPerson: cclDataList[i]?.intimationGivenTo?.person,
              intimatedDate: cclDataList[i]?.intimationGivenTo?.intimatedDate,
            },
            medicalExamination: cclDataList[i]?.medicalExamination,
            rapeCase: cclDataList[i]?.rapeCase,
            femaleArrest: cclDataList[i]?.femaleArrest,
            policeRecords: cclDataList[i]?.policeRecords,
            suretyDetails: cclDataList[i]?.suretyDetails,
            media: cclDataList[i]?.media,
          },
        };
        Object.assign(cclDataList[i], obj);
      }
    }
    setSelectedCCLDataList([]);
    setSelectedCCLDataList(cclDataList);
    // setIsRecordModalVisible(true); // open if we click on grid
  }, [
    JuvenileApprehensionData,
    JuvenileApprehensionData?.juvenileApprehensionList,
  ]);
  ...
  const handleEditArrestDetails = (value) => {
    ...
    setarrestpoliceMedia(
      value?.arrestByPolice?.media ? value?.arrestByPolice?.media : []
    );
    setInputFileList(
      value?.arrestByPolice?.media ? value?.arrestByPolice?.media : []
    );
  ...
  const createArrestbyPolicePayload = (values) => {
    if (inputFileList.length === 0 || arrestpoliceMedia.length === 0) {
      ...
      if (!isEmpty(newFileData) && !isEmpty(uploadMedicalCertificateUrl)) {
        axios
          .all([
            axios.post(
      ...
      } else if (isEmpty(newFileData) || isEmpty(uploadMedicalCertificateUrl)) {
        if (isEmpty(newFileData) && !isEmpty(uploadMedicalCertificateUrl)) {
          axios
  ...
  const filterSelectedCCLDataList = selectedCCLDataList.filter(
    (item) =>
      !getSavedDataFor41Crpc().some(
        (val) => val?.selectedRecord?.accusedId?._id === item?.accusedId?._id
      )
  );
  ...
  return (
    <ArrestWrapper>
      <ContentHeader
        headerTitle={selectedInvestigationFormObj?.label}
        addAnother
      ...
      {isFetching ? (
        ...
        <>
            ...
            <div className="widgetPageStyle">
                ...
                <Card className="cardLeftStyle">
                  ...
                  {isArrest ? (
                    <ArrestContainer
                      ...
                      viewArrestDetails={viewArrestDetails || disableForm}
              ...
              <Col span={7}>
                <Card className="right-section cardRightStyle">
                  ...
                  {selectedArrestValue === arrestTypeOption.ARREST_BY_POLICE ? (
                    ...
                    >
                      <UploadForm
                        ...
                        disabled={viewArrestDetails || disableForm}
                        ...
                        editJuvenileObj={editJuvenileObj}
                  ...
                  {!isEmpty(filterSelectedCCLDataList) ? (
                    <ArrestList
                      isDisplayed={false}
                      minHeight="650"
                      showRecords={true}
                      setviewEditObj={setviewEditObj}
                      setviewEditObjIndex={setviewEditObjIndex}
                      records={filterSelectedCCLDataList}
          ...
          {isModalVisible && (
            <TemplatesModal
              reportData={{ ...reportData, accusedPersonalDetails }}
