...
export const arrestOption = {
  ...
  ACCUSED_OUT_OF_COUNTRY: "CCL Out of Country",
};

export const arrestTypeOption = {
  ARREST_BY_POLICE: "Apprehension By Police",
  ARREST_BY_OTHER_POLICE: "Apprehension by other Police",
  ARREST_ON_ANTICIPATORY_BAIL: "Apprehension on Anticipatory Bail",
  ARREST_ON_SURRENDER_IN_POLICE_STATION:
    "Apprehension on Surrender in Police Station",
};
    ...
    if (arrestType === arrestTypeOption.ARREST_BY_POLICE) {
      ...
      switch (fileName) {
        ...
        case "Age_Determination_Requisition":
          ...
          reportData = { ...commonReportData, ...resultAgeDetermination };
          break;
        case "Apprehension_Memo":
          const resultApprehensionMemo = {
            currentDate: arrestedDateTime
              ? arrestedDateTime.format(DATE_FORMAT)
              : moment().format(DATE_FORMAT),
            hospitalName: medicalExamination?.hospitalName,
          ...
          reportData = { ...commonReportData, ...resultApprehensionMemo };
          break;
        case "XV_Juvenile_Justice_Act_2000_Notice":
          const resultJustice = {
            currentDate: arrestedDateTime
          ...
          reportData = { ...commonReportData, ...resultJustice };
          break;
        case "Night_Custody_Requisition_to_Observation_Home":
          const resultNightCustody = {
            currentDate: arrestedDateTime
              ? arrestedDateTime.format(DATE_FORMAT)
              : moment().format(DATE_FORMAT),
            hospitalName: medicalExamination?.hospitalName,
          ...
          reportData = { ...commonReportData, ...resultNightCustody };
          break;
        case "Arrest_memo":
  ...
  if (value) {
    ...
    if (value.arrestType === arrestTypeOption.ARREST_BY_POLICE) {
      ...
      form.setFieldsValue({
        ...
        relationToAccused: intimationOfArrest?.relationToAccused,
        selectPerson:
          intimationOfArrest?.selectPerson?.personalDetails?.name +
          " " +
          intimationOfArrest?.selectPerson?.personalDetails?.surname,
