...
import { consumerDetailsFields, consumerDetailsColumns } from "./const";
import {
  getActionFieldValue,
  mandatoryCheck,
  getRecordsPayload,
...
export default function ConsumerDetails({
  ...
  useEffect(() => {
    if (!isEmpty(selectedInterrogation)) {
      const { consumerDetails = [] } = selectedInterrogation;
      if (isArray(consumerDetails) && !isEmpty(consumerDetails)) {
        const oldRecords = getRecordsValue(consumerDetails);
  ...
  const columns = useMemo(() => {
    ...
    return [...consumerDetailsColumns, actions];
  }, [isViewOnlyMode, consumerDetailsColumns]);

  const onSave = useCallback(() => {
    const payload = {
      crimeId: crimeId,
      person: selectedAccused,
      consumerDetails: getRecordsPayload(records),
    };
