...
import { typeOfDrugsFields, typeOfDrugsColumns } from "./const";
import {
  getActionFieldValue,
  mandatoryCheck,
  getRecordsPayload,
...
export default function TypeOfDrugs({
  isViewOnlyMode = true,
  selectedAccused = "",
  ...
  useEffect(() => {
    if (!isEmpty(selectedInterrogation)) {
      const { typesOfDrugs = [] } = selectedInterrogation;
      if (isArray(typesOfDrugs) && !isEmpty(typesOfDrugs)) {
        const oldRecords = getRecordsValue(typesOfDrugs);
  ...
  const columns = useMemo(() => {
    ...
    return [...typeOfDrugsColumns, actions];
  }, [isViewOnlyMode, typeOfDrugsColumns]);

  const onSave = useCallback(() => {
    const payload = {
      crimeId: crimeId,
      person: selectedAccused,
      typesOfDrugs: getRecordsPayload(records),
    };
