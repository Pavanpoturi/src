...
import { SIMDetailsFields, SIMDetailsColumns } from "./const";
import {
  getActionFieldValue,
  mandatoryCheck,
  getRecordsPayload,
...
export default function SIMDetails({
  ...
  useEffect(() => {
    if (!isEmpty(selectedInterrogation)) {
      const { SIMDetails = [] } = selectedInterrogation;
      if (isArray(SIMDetails) && !isEmpty(SIMDetails)) {
        const oldRecords = getRecordsValue(SIMDetails);
  ...
  const columns = useMemo(() => {
    ...
    return [...SIMDetailsColumns, actions];
  }, [isViewOnlyMode, SIMDetailsColumns]);

  const onSave = useCallback(() => {
    const payload = {
      crimeId: crimeId,
      person: selectedAccused,
      SIMDetails: getRecordsPayload(records),
    };
