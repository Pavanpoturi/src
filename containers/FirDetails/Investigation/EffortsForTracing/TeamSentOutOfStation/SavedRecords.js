...
export default function SavedRecords({
  ...
  const columns = [
    ...
    {
      title: "Date of arrival to Place of Visit",
      dataIndex: "dateOfArrivalToPlace",
      rowKey: "dateOfArrivalToPlace",
      render: (dateOfArrivalToPlace) => (
        <span className="tableRowText wordWrap">
          {dateOfArrivalToPlace
            ? moment(dateOfArrivalToPlace).format(DATE_FORMAT)
            : ""}
        </span>
      ),
    },
    {
      title: "Place Visited",
      dataIndex: "placeVisited",
      rowKey: "placeVisited",
      render: (_i, item) => {
        const placeVisited = item?.placeVisited;
        const address1 = placeVisited?.address1 ? placeVisited?.address1 : "";
        const address2 = placeVisited?.address2 ? placeVisited?.address2 : "";
        return (
          <span className="tableRowText wordWrap">
            {address1 || ""} {address2 || ""}
          </span>
